...
write.table(DGE_QLF_results,
            file = "D40_N40_N40_V1_V2_removed/QLFTest_results_table.txt",
            quote = F, row.names = F, col.names = T,
            sep = "\t")

######## DE analysis use LR Test #########
DGE_LRT <- glmFit(DGE_Raw, # note here we use glmFit rather than glmQLFit
                  design = design_samples)
DGE_LRT <- glmLRT(DGE_LRT,
                  contrast = c(1, -1, 0)) # refer to the structure of the design matrix

table(decideTests(DGE_LRT))




## make MD plot to see data distribution
plotMD(DGE_LRT)

## export data as a data.frame
DGE_LRT_results <- DGE_LRT$table
DGE_LRT_results$Geneid <- rownames(DGE_LRT_results)
DGE_LRT_results$FDR <- p.adjust(DGE_LRT_results$PValue,
                                   method = "fdr")
...
GO_results_MF <- GO_results[GO_results$Ont == "MF", ]
GO_results_MF$FDR.Up <- p.adjust(GO_results_MF$P.Up,
                                 method = "fdr")
GO_results_MF$FDR.Down <- p.adjust(GO_results_MF$P.Down,
                                   method = "fdr")

GO_results_CC <- GO_results[GO_results$Ont == "CC", ]
GO_results_CC$FDR.Up <- p.adjust(GO_results_CC$P.Up,
                                 method = "fdr")
GO_results_CC$FDR.Down <- p.adjust(GO_results_CC$P.Down,
                                   method = "fdr")

write.table(GO_results_BP,
            file = "D40_N40_N40_V1_V2_removed/GO_results_Biological_Processes.txt",
            row.names = T, col.names = T, quote = F, sep = "\t")
write.table(GO_results_CC,
            file = "D40_N40_N40_V1_V2_removed/GO_results_Cellular_Components.txt",
            row.names = T, col.names = T, quote = F, sep = "\t")
write.table(GO_results_MF,
            file = "D40_N40_N40_V1_V2_removed/GO_results_Molecular_Functions.txt",
            row.names = T, col.names = T, quote = F, sep = "\t")

KEGG_results <- kegga(DGE_QLF,
                      species = "Hs",
                      FDR = 0.2)
write.table(KEGG_results,
            file = "D40_N40_N40_V1_V2_removed/KEGG_results_FDR_0.2.txt",
            row.names = T, col.names = T, quote = F, sep = "\t")
