...
ggplot(LINC00869_qPCR,
       ...
       shape = 0) +
  geom_boxplot(outlier.alpha = 0) +
  geom_jitter(data = LINC00869_qPCR,
              aes(colour = Cell_line),
              stat = "identity",
              # position = "jitter",
              width = 0.1,
...
ggplot(lncRNA_qPCR,
  ...
  scale_fill_manual(values = brewer.pal(n = 3,
                                        name = "Dark2")) +
  ylim(0, 3) +
  ylab("lncRNA LOC105371427 Relative Exp. x 1e-3") +
  theme_classic() +
  theme(legend.position = "none")
